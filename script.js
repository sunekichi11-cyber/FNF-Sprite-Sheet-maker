...
for (
  ...
  try {
    ...
    throw new Error(
      `${file.name}を読み込めませんでした。画像サイズやPNG形式を確認してください。`
    );
  }
}
...
loadedFrames =
  currentGroup.frames;
    ...
    alert(
      "一部の画像を読み込めませんでした。PNGファイルを確認してください。"
    );
  }
}
...
function loadImageFile(file) {
  return new Promise((resolve, reject) => {
    ...
    image.onload = () => {
      try {
        const trimData =
          calculateTransparentBounds(image);
        ...
        resolve({
          ...
          trimX: trimData.x,
          trimY: trimData.y,
          trimWidth: trimData.width,
          trimHeight: trimData.height,
